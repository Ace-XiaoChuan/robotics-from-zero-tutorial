# Lesson 9 笔记：qpos、最小二乘、雅可比矩阵和伪逆

本文行数：334行，预期阅读时长：6分钟

这份笔记用于帮助以后重新理解 Lesson 9 里 FK/IK 相关代码。重点不是推公式，而是把这些概念和机器人代码里的变量对应起来。
...
Lesson 9 里，Pinocchio 和 KDL 返回的 FK 矩阵一致，说明两边模型的运动链、关节轴和输入的 `qpos` 基本一致。
